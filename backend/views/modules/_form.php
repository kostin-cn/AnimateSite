<?php

use yii\helpers\Html;
use yii\web\JsExpression;
use yii\widgets\ActiveForm;
use vova07\imperavi\Widget;

/* @var $this yii\web\View */
/* @var $model common\entities\Modules */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="modules-form">

    <?php $form = ActiveForm::begin(); ?>
    <div class="box">
        <div class="box-body">

            <div class="row">
                <?php if (!in_array($model->id, [7, 8]) or $model->isNewRecord): ; ?>
                    <div class="col-6">
                        <?php
                        $img = ($model->image_name) ? $model->image : '/files/default_thumb.png';
                        $label = Html::img($img, ['class' => 'preview_image_block', 'alt' => 'Image of ' . $model->id]) . "<span>Изображение</span>";
                        ?>
                        <?= $form->field($model, 'uploadedImageFile', ['options' => ['class' => 'img_input_block']])
                            ->fileInput(['class' => 'hidden-input img-input', 'accept' => 'image/*'])->label($label, ['class' => 'label-img']); ?>
                    </div>
                <?php endif; ?>
                <?php if (in_array($model->id, [3, 4]) or $model->isNewRecord): ; ?>
                <div class="col-6">
                    <?= $form->field($model, 'attachments')->widget(
                        'backend\components\widget\GalleryUpload',
                        [
                            'url' => ['file-storage/upload'],
                            'sortable' => true,
                            'maxFileSize' => 10 * 1024 * 1024, // 10 MiB
                            'acceptFileTypes' => new JsExpression('/(\.|\/)(gif|jpe?g|png)$/i'),
                            'maxNumberOfFiles' => 2,
                            'clientOptions' => []
                        ]
                    ); ?>
                </div>
                <?php endif; ?>
            </div>
            <div class="row">
                <div class="col-6">
                    <?php if (!in_array($model->id, [5, 6]) or $model->isNewRecord): ; ?>
                        <?= $form->field($model, 'title_ru')->textarea(['rows' => 2, 'maxlength' => true]) ?>
                    <?php endif; ?>

                    <?php if (!in_array($model->id, [1, 3, 5, 6]) or $model->isNewRecord): ; ?>
                        <?= $form->field($model, 'html_ru')->widget(Widget::class, [
                            'settings' => [
                                'lang' => 'ru',
                                'minHeight' => 200,
                            ]
                        ]); ?>
                    <?php endif; ?>
                </div>
                <div class="col-6">
                    <?php if (!in_array($model->id, [5, 6]) or $model->isNewRecord): ; ?>
                        <?= $form->field($model, 'title_en')->textarea(['rows' => 2, 'maxlength' => true]) ?>
                    <?php endif; ?>

                    <?php if (!in_array($model->id, [1, 3, 5, 6]) or $model->isNewRecord): ; ?>
                        <?= $form->field($model, 'html_en')->widget(Widget::class, [
                            'settings' => [
                                'lang' => 'ru',
                                'minHeight' => 200,
                            ]
                        ]); ?>
                    <?php endif; ?>
                </div>
            </div>

        </div>
    </div>
    <div class="form-group">
        <?= Html::submitButton('Сохранить', ['class' => 'btn btn-success'])
        ?>
    </div>

    <?php ActiveForm::end(); ?>

</div>
